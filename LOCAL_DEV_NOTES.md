...
- Byte 13b-c3 wires musical events to persistence through that source buffer. `handleMusicalEvent` only enqueues `{ event, tonalContext }`; a setup-time interval drains up to 64 sources, builds payloads, and calls the retained persistence queue with deterministic ids shaped like `musical-<sessionId>-<sourceEventId>`. Use `window.persistence.getMusicalEventBufferState()` and `window.persistence.flushMusicalEvents()` to inspect or manually drain the source buffer.
