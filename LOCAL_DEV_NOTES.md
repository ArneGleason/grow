...
- Byte UI-3 surfaces SongGoal as the always-visible prompt front door. Use `data-testid="song-goal-frontdoor-input"`, `song-goal-frontdoor-interpret`, and `song-goal-frontdoor-apply` for browser checks; the drawer's original `song-goal-idea-input` and `song-goal-*` readouts remain mounted and synchronized after interpretation.
