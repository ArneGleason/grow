...
- Stable selectors or test IDs: Byte 10b exposes `transport-toggle`, `transport-status`, `session-mode-control`, `session-mode-current`, `session-mode-break`, `session-mode-solo-practice`, `session-mode-rehearsal`, `session-mode-performance`, `terrarium-container`, `terrarium-canvas`, `player-list`, `player-pulse-*`, `player-bass-*`, `player-melody-*`, including `player-*-expression`, `thought-seed-list`, `thought-seed-pulse-*`, `thought-seed-bass-*`, `thought-seed-melody-*`, `thought-request-pulse-*`, `thought-request-bass-*`, `thought-request-melody-*`, `thought-intent-pulse-*`, `thought-intent-bass-*`, `thought-intent-melody-*`, `ollama-base-url-input`, `ollama-model-input`, `ollama-health-check`, `ollama-send-thought`, `ollama-health-status`, `ollama-model-status`, `ollama-latency`, `ollama-parse-result`, `ollama-validation-result`, `ollama-fallback-status`, `ollama-errors`, `ollama-primer-summary`, `ollama-raw-response`, `listening-event-count`, `listening-window`, `listening-latest-event`, `lookahead-health`, `lookahead-lead`, `lookahead-through`, and `lookahead-pending-slots`.
...
- Byte 9b implements `src/ollama.ts` as a browser-side local Ollama boundary. It talks to `GET /api/tags` and `POST /api/chat` on `VITE_GROW_OLLAMA_BASE_URL` with `stream: false` and JSON-format responses.
- The default local model tag is `gemma4:31b`, configurable in `.env.local`, the inspector model input, or `window.ollama.setConfig({ model })`. Adjust it to the exact tag returned by `ollama list` if needed.
