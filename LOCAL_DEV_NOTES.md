...
- Byte E3a's focused listening-vote smoke is `npx playwright test tests/grow.smoke.spec.ts -g "interplay bass answers"`. It waits through the color-on and color-off chorus passes, then verifies A/B take arming plus `song.interplay_vote` persistence. If a full parallel smoke shows unrelated editor/transport timing flakes, rerun the failed slice and then a confirmation full smoke before treating it as a regression.
