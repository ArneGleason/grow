...
- Visual regression entry points: capture the Vite root page at `http://127.0.0.1:5173/`; the terrarium canvas should show four gently drifting players: `pulse`, `bass`, `keyboard`, and `melody`.
...
- Track D5 adds default-off evolving performance. From the browser console, use `window.prosody.performEvolvingElite({ seed: 4242, branchId: "d5-live-4242", generations: 50, batch: 2, intervalMs: 350, count: 5, eliteLimit: 3, diversity: { enabled: true, fitnessEliteLimit: 1, minDistance: 0.12, reservoirLimit: 2, reservoirParentFraction: 0.5, interestingnessThreshold: 0.35 } })`, then Start playback. Inspect with `window.prosody.getEvolvingPerformance()` and stop/restore default melody with `window.prosody.stopEvolvingElite()`.
- Byte UI-2 adds the top-bar Line dial as the user-facing master control. `0..0.33` is `written` (prosody off, evolving stopped), `0.34..0.67` is `speaking` (prosody on, evolving stopped), and `0.68..1` is `evolving` (D5 runs on `branchId: "dial-<songId>"` with a deterministic per-song seed and diversity enabled). Console helpers: `window.prosody.getControlDial()` and `window.prosody.setControlDial(0.5)` / `setControlDial(0.85)`.
- Byte connector-color-a adds `PatternNoteSource.chromaticOffsetSemitones` and optional note `tags`. Use `noteFromScaleDegreeWithChromaticOffset()` when turning pattern notes into pitches; transport and MIDI export both depend on it so colored connector notes sound/export the same way. `scaleDegree` remains the modal structural identity for scoring/listening, while `connector:chromatic` tags flag out-of-scale connector color in heard events.
- Byte sound-preview-a adds `window.sound.getMix()`, `setMasterLevel(level)`, `setPlayerLevel(playerId, level)`, and `setPlayerVoice(playerId, voice)` for manual preview mix control. Pulse defaults to `drum-kit`; heard pulse events include `pulse:drum-kit`/`drum:*`/`gm:*` tags, and MIDI export reports pulse on zero-indexed channel `9` (standard MIDI channel 10 percussion).
- Byte starter-musicality-a makes starter-generated material 32 beats long. `createSongStarterMaterial()` now creates an eight-bar pulse/bass/melody pack, while `createChorusMelodyEvent()` maps fallback chorus hooks over 16 beats at half-beat resolution against the `answer` root plan. If a test assumes starter pattern length or chorus hook repetition, check for `32`-beat starter patterns and 16-beat chorus phrases.
- Byte workflow-clarity-a hides starter-only song-strip controls until the active library song has a generated starter, and the starter composer exposes step state through `data-testid="song-starter-step-describe"`, `song-starter-step-generate`, `song-starter-step-create`, plus `song-starter-status`. Tests that need Edit/Regen should create a prompt-generated song first.
- Voice-led harmony draft A adds `src/voice-led-harmony.ts` plus `window.harmonyDraft.generate()` / `window.harmonyDraft.getCurrent()`. The first slice was inspect-only; the audible starter path now consumes the same draft through `createSongStarterMaterial()`, tagging generated pulse/bass/melody notes with `starter:voice-led-harmony`. Use `npm run unit:voice-led-harmony` for the focused pure contract and the song-library smoke to confirm the transport hears those tags.
- Song-builder language cleanup keeps the old `song-starter-*` test ids and internal names, but the visible path is now `Write song -> Make draft -> Use this song -> Start`. Visible copy should avoid `starter`, `material seed`, and `template` except in internal docs/tests. The dirty-DB new-idea smoke should not expect candidate-list length growth when `listCandidates({ limit: 500 })` is already capped.
- Audible voice-led starter integration replaces the old fixed root-cycle/connector-first melody as the primary generated song material. `src/song-starter-material.ts` now derives bar roots, bass, pulse cues, and melody guide/counter notes from `generateVoiceLedHarmonyDraft()`. The draft generator also has multiple contour, guide-rhythm, counter-interval, cadence, and upper-counter placement families; unit coverage asserts several seeds produce distinct contour and harmonic-rhythm signatures.
- Keyboard-player-a adds `keyboard` to the main player registry with default preview voices (`warm-keys`, `soft-piano`, `muted-organ`). Built-in songs now include compact three-lane keyboard comping, and starter-generated songs turn voice-led chord events into three simultaneous keyboard pattern lanes tagged `starter:voice-led-harmony` and `keyboard:comp`. MIDI export maps keyboard to zero-indexed channel `3`.
- Song-library-curation-a adds foreground library pruning. The Prune action removes only the current local song-library entry, never the last fallback song, and applies the next selected song through the normal song-context change path. Use `window.songLibrary.prune()` for console inspection.
- Melody-harmony-binding-a makes starter melody anchors consume the same `HarmonyChordEvent` stream as bass/keyboard. Strong melody notes carry `melody:harmony-bound` and `melody:chord-tone`; upper/color connector notes carry `melody:resolves-to-chord`. Use `npm run unit:song-starter-material` for the focused relational contract.
- LLM song-intent-a adds `src/song-intent.ts` and a song-builder model interpretation pass. The app only calls `/api/ollama/chat` for song intent after `window.ollama.checkHealth()` has made health `ready`; otherwise `Make draft` stays on the deterministic fallback. Inspect with `window.ollama.getLastSongIntentTest()` and `window.ollama.getSongIntentPrompt()`. Focused validation: `npm run unit:song-intent` and `npm run smoke -- --grep "song starter applies bounded Ollama song intent" --workers=1`.
- LLM melody/harmony draft A adds `src/song-draft-plan.ts`. When Ollama health is ready, `Make draft` now makes a second bounded `/api/ollama/chat` call for an eight-bar co-draft plan after song intent. Inspect with `window.ollama.getLastSongDraftPlanTest()` and `window.ollama.getSongDraftPlanPrompt()`. Saved generated songs may carry `starter.draftPlan`; `createSongStarterMaterial()` renders it through the existing voice-led harmony path and heard/generated notes carry `song-plan:model`. Focused validation: `npm run unit:song-draft-plan` and `npm run smoke -- --grep "song starter applies bounded Ollama song intent and co-draft plan" --workers=1`.
...
- Playwright smoke has `reuseExistingServer: true`; if a focused smoke sees old UI copy or missing new test IDs, check for a stale Vite server on `127.0.0.1:5173` before trusting the failure.
