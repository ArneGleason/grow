...
Status: Byte 13a prep. This document defines durable shapes and boundaries only. It does not introduce SQLite, browser storage, persistence writes, or replay behavior.
...
- Create the local backend-owned SQLite file.
- Write `sessions` and append `events` for one or two safe record types.
