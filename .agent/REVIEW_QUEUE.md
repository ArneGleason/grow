...
- Byte 15c-a is ready for review on `codex/byte-15c-a`: deterministic band consensus now sits between a model/deterministic proposal and the active chorus candidate. Review focus: the model still emits only candidate id plus prose; consensus selects only from app-owned candidates; `spacious-hook` can pass when proposed, while weak `lifted-hook` is not rubber-stamped; invalid model output falls back to deterministic consensus; and the inspector/persistence clearly separate proposed id from consensus-selected id.
- Next Byte 15 direction after review: remember-good should use the stored proposal/consensus/feedback trail rather than only nudging a generic surprise weight.
