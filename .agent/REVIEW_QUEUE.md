...
- Before the next Ollama behavior byte, remove `pitch` from the model-facing step schema and have the system derive pitch from `scaleDegree` plus `octave`; real qwen3 sometimes emitted pitch/degree disagreement, and the validator correctly rejected it.
- Before the automatic slow-thinking loop, add upstream abort propagation for proxied Ollama calls, mocked invalid/unavailable Ollama tests, and surface available models as a picker.
