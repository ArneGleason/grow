...
- Byte 13b-a is approved and merged. Byte 13b-b should wire only low-frequency UI decision records (`session.started`, `session.mode_changed`, `song.changed`, `timing.feel_changed`) through a buffered/off-audio-path writer. Hold `musical.event_recorded` for 13b-c because it is high-frequency and scheduler-adjacent.
