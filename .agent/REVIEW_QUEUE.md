...
- Byte 12b-a forward notes before or during proposal/response work: derive the harmonic source by bass role instead of literal player id, keep section split length and section duration length aligned if future patterns have different longest loops, and deep-clone/freeze/comment cached nested sketch arrays before returned sketches can be mutated.
