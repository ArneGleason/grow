...
- Byte 13b-c1 is ready for review. Focus on the degraded persistence path: no-db dump/status should not create empty DB files, retries should be bounded/idempotent, pagehide flush should be explicit and non-destructive to the queue, the visible status line should make degraded writes legible, and no persistence writes should enter `handleMusicalEvent` or Tone callbacks.
- Next persistence slice after 13b-c1 should be `musical.event_recorded` behind a high-frequency ring buffer + batch flush discipline, with tests proving Tone callbacks only enqueue memory data and never write/fetch directly.
