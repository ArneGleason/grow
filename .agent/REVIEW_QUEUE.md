...
- Byte 13a is approved and merged. Byte 13b should carry Claude's design-completeness additions into the first SQLite slice: add `song.changed` and `timing.feel_changed` record types, include taste action-dwell state in seek-and-continue checkpoints, and mark listening frame/agitation/contagion/taste display evaluations as ephemeral derived state.
