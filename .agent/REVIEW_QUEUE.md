...
- Byte 12a is ready for review. Focus on the inspect-only `SongSketch` shape, whether it reads as a band-level draft rather than a private player intent, and whether it is small enough to evolve into practice/reference/persistence later.
- Byte 12a also folds in the prompt cleanup from Byte 11e: the `registerDelta` instruction sentence should now be absent from non-shift system/user prompts while still present for melody shift-capable calls.
