...
- Byte 13b-b is ready for review. Focus on whether the browser persistence queue and `/api/persistence/*` Vite middleware stay clearly off the audio scheduler path, whether the four low-frequency records are enough for this slice, whether `beat`/`scheduled_beat` naming is now correct, and whether failure remains soft if persistence is unavailable.
