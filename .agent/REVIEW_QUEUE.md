...
- Byte 10b implementation review focus: confirm `src/expression.ts` is pure/replayable, final velocities and event metadata are bounded, rest events do not sound, latest expression state clears on stop, and the transport did not accrete non-velocity behavior ahead of Byte 10c.
