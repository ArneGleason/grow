...
- Byte 10f-b2 review should check that the model-facing schema truly omits `pitch`, system-side derivation cannot mask out-of-range `scaleDegree`, proxy abort propagation is sufficient for future cancelled slow thoughts, and mock fallback remains visible when proxy chat fails.
- Before the automatic slow-thinking loop, surface available models as a picker and decide whether any further invalid-response fixtures are needed beyond the current unavailable/proxy-failure smoke.
