...
- Byte 11e is ready for review. Focus on whether gating the `registerDelta` schema property on `allowedActions.includes("shift_register")` is enough to prevent bass/non-shift leakage while preserving melody shift-register compliance.
- Byte 11e review should confirm the validator still rejects stray `registerDelta` on non-shift actions and that the projected format for bass contains neither `registerDelta` nor `shift_register` under `additionalProperties:false`.
