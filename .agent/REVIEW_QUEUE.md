...
- Byte 13b-a is ready for review. Focus on whether the Node `node:sqlite` shell is the right first persistence boundary, whether `server/persistence.mjs` stays generic enough for the later local backend writer, whether `scripts/grow-db.mjs` gives reviewers enough inspectability, and whether app/runtime behavior remains unwired.
