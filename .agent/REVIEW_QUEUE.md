...
- Byte 11d is ready for review. Focus on the multiple-controller scheduling policy, the per-player active playback map, the bass action restriction to rest/thin only, and whether the new plural debug API is the right compatibility bridge.
- Byte 11d review should verify no surprise local model concurrency: the app should keep at most one slow-thinking Ollama request pending globally even though melody and bass each have a controller.
