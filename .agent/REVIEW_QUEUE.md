...
- Byte 12b-b is ready for Claude review. It adds deterministic inspect-only `SongSketchProposal`/responses, renders the new Song Sketch rows, exposes `window.song.getProposal()`, resolves bass-role harmonic lookup, and clones nested sketch returns; no playback/transport/slow-thinking path should have changed.
- Byte 12b-b review focus: confirm the proposal/response shape is the right band-level surface before any model-authored or audible songcraft, and confirm keeping section durations as full-loop overlays while root splitting uses the harmonic source's own loop is acceptable.
