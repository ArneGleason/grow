...
- Optional final pre-Byte-12 cleanup: the static `registerDelta` prompt sentence is still emitted for non-shift lanes even though the schema now omits the property. Gate that sentence on the same `allowsRegisterShift` condition, ideally with a shared conditionally included constant.
