...
- Byte 13b-c3 required one fix before merge: `musical-<sessionId>-<sourceEventId>` collided across stop/start spans because `sourceEventId` resets with transport-local `eventSerial`. Byte 13b-c4 is ready for review with ids shaped `musical-<sessionId>-span-<playSpanSerial>-<sourceEventId>` and a two-span smoke requiring persisted musical rows to equal total drained source events. Drops still have no persisted gap marker.
