...
- Byte 13b-c2 is ready for review. Focus on whether `MusicalEventRecordPayload` has the right grid/performed split for replay, whether keeping `MusicalEvent.pitch` as performed-pitch alias is acceptable compatibility, whether the drop-oldest ring buffer is the right back-pressure policy before DB flushing, and whether this byte successfully avoids any SQLite/fetch/timer wiring from musical events.
