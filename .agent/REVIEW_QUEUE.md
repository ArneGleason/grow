...
- Byte 6a is approved. Current Codex next bite: Byte 6b break-vs-rehearsal behavior should wire session mode into transport and make break stop new lookahead commits while already-queued material drains.
- Byte 6a forward nits: derive the static default mode label/radio state from `DEFAULT_SESSION_MODE`, and share the duplicated set-mode/render helper between DOM listener and `window.session.setMode`.
