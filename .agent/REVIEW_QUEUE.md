...
- Byte 13b-c3 should wire musical events into the buffer plus separate batch flusher. Review focus: callback push should stay minimal and synchronous; prefer pushing event + tonal snapshot and building payload in the flusher; drained-but-failed batches must go to the retained/idempotent queue keyed by `sourceEventId`, not back into the ring; stop/cleanup flush-or-discard must be a tested decision; persisted `seq` will be contiguous-on-write, so source gaps live in eventIndex/absoluteBeat and may need a persisted gap marker later.
