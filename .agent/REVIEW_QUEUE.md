...
- Byte 15b-a review focus: confirm the model critic cannot emit or mutate notes, pitches, scores, timing, or section structure; valid output only selects a local candidate id; invalid/failed output leaves the deterministic scorer active; and a model-selected alternate actually changes the audible chorus candidate through the lookahead material path.
